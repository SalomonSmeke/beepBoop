...
void select(byte mask) {
  ...
  while (menuTimeOut) {
    ...
    if (!input) {
      ...
      if (mode == BH) {
        setHalf(yoff, *corner, *(corner + 1), *(corner + 2));
      } else {
...
void setHalf(byte yoff, byte r, byte g, byte b) {
  byte xlim = 8;
  byte ylim = yoff + 4;
  for (byte i = yoff; i < ylim; i++) {
    for (byte a = 0; a < xlim; a++) {
      setPixel(a, i, r, g, b);
    }
  }
}
...
void initMatrix() {
  ...
  for (byte y = 0; y < 4; y++){
    for (byte x = 0; x < 2; x++){
      setPixel(x, y, 127, 0, 0);
    }
    for (byte x = 2; x < 4; x++){
      setPixel(x, y, 127, 0, 60);
    }
    for (byte x = 4; x < 6; x++){
      setPixel(x, y, 60, 0, 127);
    }
    for (byte x = 6; x < 8; x++){
      setPixel(x, y, 0, 0, 127);
    }
  }
  for (byte y = 4; y < 8; y++){
    for (byte x = 0; x < 4; x++){
      setPixel(x, y, 127, 127, 0);
    }
    for (byte x = 4; x < 8; x++){
      setPixel(x, y, 0, 127, 127);
    }
  }
  refresh();
}
